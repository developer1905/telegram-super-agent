...
## 2. Telegram Mini App (Web App) va API Xavfsizligi
...
3. **Zero Client Trust:** Frontend tomonidan yuborilgan soxta `user_id` larga ishonilmaydi. Foydalanuvchi identifikatori faqat serverda tasdiqlangan `initData` ichidan olinadi (`request["authenticated_user_id"]`).
4. **PUBLIC_PATHS Minimalligi:** Faqat haqiqiy ommaviy resurslar (`/`, `/health`, `/readiness`, `/webapp`, `/landing`, `/favicon.ico`) ochiq qoldirilgan. Barcha `/api/*` endpointlari autentifikatsiya talab qiladi (401).
5. **RBAC & Authorization:** Admin marshrutlari (`/api/switch_model`, `/api/scheduled_posts`, `/api/system_info`, `/api/managed_chats`, `/api/uptime/*`, `/api/clean_server`) faqat tasdiqlangan admin tomonidan chaqirilishi mumkin; oddiy foydalanuvchilar uchun `403 Forbidden` qaytariladi.
6. **Iframe & CSP:** Brauzerlararo iframe yuklanishini xavfsiz ta'minlash uchun `Content-Security-Policy: frame-ancestors 'self' https://web.telegram.org https://*.telegram.org https://telegram.org;` qo'llanilgan.

---

## 3. Ko'p Foydalanuvchilik va Ma'lumotlar Izolyatsiyasi (Multi-Tenancy)
1. **User Ownership:** `knowledge_base`, `tasks`, `uptime_monitors`, `reminders` jadvallarida har bir yozuv qat'iy ravishda `user_id` ga bog'langan.
2. **Composite Key:** `knowledge_base` jadvalida `UNIQUE(user_id, key)` kompozit cheklovi orqali foydalanuvchilar o'rtasida kalitlar to'qnashuvi va bir-birining faktlarini qayta yozib yuborish holatlari bartaraf etilgan.
3. **Astrology Privacy:** `/api/astrology/profile` endpointi faqat autentifikatsiya qilingan foydalanuvchining o'z profilini ko'rishga ruxsat beradi (boshqa user profiliga so'rov `403 Forbidden`). Foydalanuvchi profili mavjud bo'lmasa, qat'iyan `404 Not Found` qaytariladi (hech qachon boshqa foydalanuvchining so'nggi profili berilmaydi).
...
1. **Kontekstlar Bo'linishi:**
   ...
   - `GROUP_CONTEXT`: Guruhlarda shaxsiy xotira (factlar, eslatmalar) AI so'rovlariga qo'shilmaydi (`build_rag_context` guruh va kanallar uchun darhol bo'sh satr qaytaradi).
...
## 5. Kod Sandboxingi (Code Execution Security)
...
3. **No Unsafe Host Fallback:** Agar serverda Docker o'rnatilmagan yoki ishlamayotgan bo'lsa, kod hech qachon to'g'ridan-to'g'ri xost tizimida (`subprocess`/`eval`) bajarilmaydi, xavfsiz xatolik qaytariladi (Fail-Closed).

---

## 6. Arxiv va Fayllar Xavfsizligi (Zip Slip Protection)
1. **Path Sanitization:** `core/project_builder.py` da `sanitize_archive_path` funksiyasi `..` (directory traversal), mutlaq yo'llar (`/etc/`, `C:\`), disk harflari va NUL baytlarni (`\0`) to'liq rad etadi.
2. **Safe Extraction:** `safe_extract_zip` funksiyasi har bir arxiv a'zosining haqiqiy yo'li mo'ljallangan papka ichida qolishini kafolatlaydi.

---

## 7. Email va Tashqi Xabarlar Xavfsizligi
